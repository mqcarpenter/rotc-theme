 ...
 * The actual homepage body -- a real-league snapshot pulled from
 * /manage/api/wp-feed.php (interactive Game-of-the-Week + tiles,
 * mirroring /manage/templates/weekly-recap-hub.php's pattern) up top,
 * then CMS-controlled content (latest posts, hero + featured strip +
 * hub-style river) below.
 *
 * Included from BOTH front-page.php and page-template/home-page.php --
 * the latter exists because the site's actual "Home" page (post ID 2)
 * has a leftover custom Page Template assignment from seos-football
 * (`page-template/home-page.php`) that a static front page's own
 * explicit template takes priority over front-page.php entirely,
 * confirmed live via the page's REST API `template` field. Rather
 * than depend on someone reassigning that page back to "Default
 * template" in wp-admin (whose Page Attributes panel doesn't even
 * show a Template dropdown when the active theme registers zero
 * custom templates of its own), this theme just also provides a file
 * at that exact assigned path -- so the homepage renders correctly
 * either way, with one shared source of truth for what it looks like.
 */
