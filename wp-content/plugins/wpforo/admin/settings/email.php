...
<div class="wpf-subtitle">
    <span class="dashicons dashicons-admin-users"></span> <?php _e( 'After User Delete Notification Email for Administrators', 'wpforo' ) ?>
    <p class="wpf-desc" style="padding: 5px 2px 0;"><?php _e( 'This message comes when a user deletes self account from site', 'wpforo' ) ?></p>
</div>
<?php
WPF()->settings->form_field( 'email', 'after_user_delete_notification_email_admin_subject' );
WPF()->settings->form_field( 'email', 'after_user_delete_notification_email_admin_message' );
?>
